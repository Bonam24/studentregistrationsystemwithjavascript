...
app.delete('/students/:id', (request, response) => {
    const id = parseInt(request.params.id);
    const studentIndex = data.findIndex(student =>parseInt(student.id) === id);
    if (studentIndex !== -1) {
...
//code to update
app.put('/update/:recordid', jsonParser, (request, response) => {
    const requestBody = request.body;
    const requestId = request.params
    console.log({requestBody, requestId})
    const index = data.findIndex(data =>parseInt(data.id)  == parseInt(requestId.recordid) );
    console.log(index)
